...
async def list_customers(with_deleted: bool = False, session=Depends(get_session)):
...
async def get_customer(customer_id: UUIDType, with_deleted: bool = False, session=Depends(get_session)):
...
async def update_customer(customer_id: UUIDType, payload: CustomerUpdate, session=Depends(get_session)):
...
async def delete_customer(customer_id: UUIDType, session=Depends(get_session)):
    customers = await session.execute(
        select(CustomerModel).where(CustomerModel.id == customer_id, CustomerModel.deleted_at.is_(None))
    )
    customer = customers.scalars().first()
